...
class Proc(object):
    ...
    @property
    def cwd(self):
        return os.path.realpath('/proc/{}/cwd'.format(self.pid))

    @property
    def root(self):
        return os.path.realpath('/proc/{}/root'.format(self.pid))

    @property
    def status(self):
        return open('/proc/{}/status'.format(self.pid)).read()

    @property
    def cmdline(self):
        return open('/proc/{}/cmdline'.format(self.pid)).read()

    @property
    def cmdline_list(self):
        result = self.cmdline.split('\0')
        result.remove('')
        return result

    @property
    def username(self):
        real, effective, saved = _uids_re.findall(self.status)[0]
        try:
    ...
    @property
    def name(self):
        return _name_re.findall(self.status)[0]
